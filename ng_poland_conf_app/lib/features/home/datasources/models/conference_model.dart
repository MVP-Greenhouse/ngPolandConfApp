...
@freezed
@HiveType(typeId: 1)
abstract class ConferenceModel with _$ConferenceModel {
  ...
  factory ConferenceModel.fromJson(Map<String, dynamic> json) =>
      _$ConferenceModelFromJson(json['fields']);
          ...
          ? listItems
                .map(
                  (conferenceHomePageScheduleItemModel) =>
                      conferenceHomePageScheduleItemModel.toEntity(),
                )
                .toList()
          : [],
    );
...
List<ConferenceHomePageScheduleItemModel> _listItemsFromJson(
  ...
  var items = json?['items']
      ...
      .map<ConferenceHomePageScheduleItemModel>(
        (Map<String, dynamic> itemInJson) =>
            ConferenceHomePageScheduleItemModel.fromJson(itemInJson),
      )
      .toList();

  if (items == null && items is! List<ConferenceHomePageScheduleItemModel>)
    return [];
